...
def _enum_local_ips(os_ops: OsOperations) -> typing.List[tagIP]:
    assert isinstance(os_ops, OsOperations)

    cmd = ["ip", "-4", "addr", "show", "dev", "lo"]

    res = os_ops.exec_command(cmd, encoding="utf-8")
...
"""
    ...
    def test_is_port_available__true(
        ...
        local_ips = _enum_local_ips(os_ops_descr.os_ops)
        assert type(local_ips) is list
        ...
        for i in range(len(local_ips)):
            ...
            __class__.helper__is_port_available__true(
                os_ops_descr,
                local_ip,
            )
            continue
    ...
    # --------------------------------------------------------------------
    @staticmethod
    def helper__is_port_available__true(
        os_ops_descr: OsOpsDescr,
        local_ip: tagIP,
    ):
        assert type(os_ops_descr) is OsOpsDescr
        ...
        py_test_code_templ = """
import socket
with socket.socket(socket.{0}, socket.SOCK_STREAM) as s:
    try:
        s.bind(({1!r}, {2!r}))
    except OSError:
        exit(123)
...
"""
        ...
        for port in ports:
            ...
            py_test_code = py_test_code_templ.format(
                local_ip.family.name,
                local_ip.value,
                port,
            )
    ...
    def test_is_port_available__false(
        ...
        for i in range(len(local_ips)):
            ...
            __class__.helper__is_port_available__false(
                os_ops_descr,
                local_ip,
            )
            continue
    ...
    # --------------------------------------------------------------------
    @staticmethod
    def helper__is_port_available__false(
        os_ops_descr: OsOpsDescr,
        local_ip: tagIP,
    ):
        assert type(os_ops_descr) is OsOpsDescr
        ...
        py_server_code_templ = """
import socket, time
s = socket.socket(socket.{0}, socket.SOCK_STREAM)
try:
    s.bind(({1!r}, {2!r}))
    s.listen(1)
    print("READY", flush=True)
...
"""
        for port in ports:
            ...
            py_server_code = py_server_code_templ.format(
                local_ip.family.name,
                local_ip.value,
                port,
            )
